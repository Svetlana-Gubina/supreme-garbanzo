...
const htmlPages = () => {
  return gulp
    ...
    .pipe(gulp.dest(dir.build + "pages/"))
    .pipe(sync.stream());
};
...
const copy = () => {
  return gulp
    .src([dir.src + "js/**", dir.src + "fonts/**/*", dir.src + "img/**/*", dir.src + "*.ico"], {
      base: dir.src,
    })
    .pipe(gulp.dest(dir.build))
    .pipe(
...
const watch = () => {
  gulp.watch([dir.src + "*.html", dir.src + "pages/" + "*.html"], gulp.series(html, reload));
...
const dev = gulp.series(
  gulp.parallel(html, htmlPages, styles, normalize, scripts, images, toWebp, copy),
